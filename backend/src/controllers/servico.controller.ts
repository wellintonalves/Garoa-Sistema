...
export class ServicoController {
  ...
  static async listar(req: AuthRequest, res: Response): Promise<void> {
    try {
      const servicos = await ServicoService.listarTodos(req.usuario?.barbeariaId || undefined);
      res.json(servicos);
    } catch (error) {
  ...
  static async criar(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const servico = await ServicoService.criar(req.body, req.usuario?.barbeariaId || undefined);
